...
# mocap 动捕实时位姿真机链路（Zenoh 通讯版，无 ROS）。
#
# 复用已运行的动捕实时仿真主机（pixi run sim_mocap_live：Motive
# 实时位姿驱动 + IK），再启动 Marvin 安全桥，把 IK 解算的关节命令
# 下发到真机。Motive 手腕位姿 → 参考增量 → IK → 真机。
#
# 前置：
#   - 先运行 pixi run sim_mocap_live（动捕实时驱动 + IK 主机链路）；
#   - 必须显式 --confirm-real；
...
#   pixi run real_mocap_live -- --confirm-real --robot-ip 192.168.1.190
...
usage() {
  cat <<'EOF'
用法：
  pixi run real_mocap_live -- [--confirm-real] [--robot-ip IP]
...
register_teleop_process_group \
  "${bridge_pid}" marvin-mocap-live-hardware-bridge 30

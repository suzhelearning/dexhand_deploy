...
if ! router_zid="$(require_router)"; then exit 1; fi
export TIANJI_ROUTER_ZID="${router_zid}"
export TIANJI_COORDINATOR_INSTANCE_ID="${coordinator_id}"
export TIANJI_ACTIVE_SIDES="${active_sides}"
export TIANJI_INACTIVE_SIDES="${inactive_sides}"
export TIANJI_REQUIRED_CAPABILITY="${required_capability}"
export TIANJI_HAND_MODE="${hand_mode}"
...
[[ "${hand_mode}" == direct ]] && hand_producer_id="joint_replay"
...
base_env=("TIANJI_ROUTER_ENDPOINT=${TIANJI_ROUTER_ENDPOINT}" "TIANJI_ROUTER_ZID=${TIANJI_ROUTER_ZID}" "TIANJI_COORDINATOR_INSTANCE_ID=${coordinator_id}" "TIANJI_RUN_ID=${run_id}")
if [[ -n "${record_path}" ]]; then
...
launch coordinator "${base_env[@]}" TIANJI_COORDINATOR_INSTANCE_ID="${coordinator_id}" python "${BUNDLE_ROOT}/src/pico_body_tianji/scripts/arm_command_coordinator"
...
if [[ -n "${arm_producer_config}" && "${arm_producer_config}" != null ]]; then
...
source_args=("${base_env[@]}" TIANJI_COMPONENT_INSTANCE_ID="${source_instance}" bash "${SCRIPT_DIR}/run_source.sh" --source "${source_id}" --config "$(canonical_config "${source_config}")")
if [[ "${source_id}" == h5_replay ]]; then source_args+=(-- "${input_path}")
elif [[ "${source_id}" == target_replay || "${source_id}" == joint_replay ]]; then source_args+=(-- "${input_path}")
fi

...
case "${executor_id}" in
  mujoco)
    args=(--headless --coordinator-instance-id "${TIANJI_COORDINATOR_INSTANCE_ID}" --publisher-instance-id "${TIANJI_COMPONENT_INSTANCE_ID}")
  ...
  wuji_hand2)
    ...
    exec python "${entry}" --mode "${mode}" --side "${side}" "$@"
    ;;
esac

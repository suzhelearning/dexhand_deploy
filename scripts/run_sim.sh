...
PREVIEW_LAUNCH="${PROJECT_PREFIX}/share/pico_body_tianji/launch/preview.launch.py"
MUJOCO_VIEWER="${BUNDLE_ROOT}/src/pico_body_tianji/scripts/mujoco_joint_viewer.py"

for required in "${ROS2_BIN}" "${PREVIEW_LAUNCH}" "${MUJOCO_VIEWER}"; do
  if [[ ! -f "${required}" ]]; then
...
setsid python "${ROS2_BIN}" launch pico_body_tianji preview.launch.py \
  "with_rviz:=${with_rviz}" "${FORWARDED_ARGS[@]}" &
LAUNCH_PID=$!
register_teleop_process_group "${LAUNCH_PID}" ros-preview 5

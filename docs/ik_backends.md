...
`arm_ik_solver_v1`），不再直接依赖某一种 IK 实现。当前注册了两个后端：
...
两种后端复用完全相同的 ROS 输入、关节输出、回零轨迹和真机安全桥。
切换后端不需要修改这些节点。

## 接口契约

公共接口位于
`include/pico_body_tianji/arm_ik_solver.hpp`，固定约定如下：
...
在 `config/preview.yaml` 或 `config/controller_only_ik.yaml` 中设置：

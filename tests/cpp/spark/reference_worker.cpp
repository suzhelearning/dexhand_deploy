...
#include "reference_viewer_cycle.hpp"
#include <charconv>
#include <iomanip>
#include <iostream>
#include <sstream>
...
int main(int argc, char** argv) {
  try {
    if (argc != 4 && argc != 5) throw std::invalid_argument(
        "usage: spark_native_worker CONFIG MODEL URDF [--deterministic-test]");
    const bool deterministic = argc == 5;
    if (deterministic && std::string(argv[4]) != "--deterministic-test") {
      throw std::invalid_argument("unknown worker option");
    }
    ...
    ReferenceViewerCycle cycle(config, argv[2], argv[3], true);
    char buffer[2048];

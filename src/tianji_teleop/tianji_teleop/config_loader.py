...
def canonical_config_root() -> Path:
    ...
    以包所在的 ``src/tianji_teleop/config`` 为默认值。不会回退到旧
    ``config/mode`` 树。
    """
    ...
    bundle = os.environ.get("TIANJI_TELEOP_BUNDLE_ROOT")
    if bundle:
        candidates = (
            Path(bundle) / "runtime" / "tianji_teleop" / "share" / "tianji_teleop" / "config",
            Path(bundle) / "src" / "tianji_teleop" / "config",
            Path(bundle) / "config",
        )

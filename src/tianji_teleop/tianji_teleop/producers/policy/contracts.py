...
class ActionAdapter:
    ...
    def _load_default_step() -> float:
        bundle_root = os.environ.get("TIANJI_TELEOP_BUNDLE_ROOT")
        if bundle_root:
            path = Path(bundle_root) / "runtime" / "tianji_teleop" / "share" / "tianji_teleop" / "config" / "coordinator" / "arm.yaml"
        else:
            path = Path(__file__).resolve().parents[5] / "src" / "tianji_teleop" / "config" / "coordinator" / "arm.yaml"
        try:
            import yaml

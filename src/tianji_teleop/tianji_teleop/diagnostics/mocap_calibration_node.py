...
from ..sources.common.wrist_pose_frame import WristPoseFrame
...
class MocapLiveNode:
        ...
        if event == "s":
            ...
            if phase == "armed":
                ...
                try:
                    ...
                    command_frame = WristPoseFrame.from_poses(
    ...
    def _command_frame(self) -> WristPoseFrame | None:
        """返回冻结参考 + 键盘增量构成的虚拟帧。"""
        with self._command_lock:
            if self._accumulators is None:
                return None
            return WristPoseFrame.from_poses(
    ...
    def _tick(self) -> bool:
        ...
        with self._phase_lock:
            ...
            if command_frame is None:
                return True
            try:
                targets = self._mapper.map_relative_wrist_frame(command_frame)

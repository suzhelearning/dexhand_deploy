...
#include "tianji_teleop/ik/tianji_official/tianji_official_ipc.hpp"
...
using tianji_teleop::official_ipc::Request;
using tianji_teleop::official_ipc::Response;

tianji_teleop::ArmSide side(std::int32_t value)
{
  return value == 0 ? tianji_teleop::ArmSide::kLeft :
...
tianji_teleop::ArmJointVector joints(const double input[7])
{
  tianji_teleop::ArmJointVector output;
...
tianji_teleop::ArmJointVector parse_joints(const std::string & text)
{
  tianji_teleop::ArmJointVector output;
...
int main(int argc, char ** argv)
{
  ...
  try {
    ...
    tianji_teleop::TianjiOfficialArmIk solver(argv[2], argv[3], settings);
    while (true) {
      Request request;
      ...
      if (
        request.magic != tianji_teleop::official_ipc::kMagic ||
        request.version != tianji_teleop::official_ipc::kVersion)
      {
        return 4;
      }
      if (request.operation == tianji_teleop::official_ipc::Operation::kShutdown) {
        break;
      }
      Response response;
      try {
        if (request.operation == tianji_teleop::official_ipc::Operation::kForward) {
          copy_pose(response.pose, solver.forward(side(request.side), joints(request.joints_rad)));
        } else if (request.operation == tianji_teleop::official_ipc::Operation::kSolve) {

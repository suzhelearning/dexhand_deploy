...
void run_side(
  tianji_teleop::ArmIkSolver & solver,
  ...
  std::vector<double> & solve_times_us)
{
  ...
  for (const Scenario & scenario : scenarios) {
    ...
    int transient_otg_rejections = 0;
    for (int tick = 0; tick < 500; ++tick) {
      ...
      if (!last.accepted || !last.joints_rad.allFinite()) {
        // Original OTG can transiently reject a static combined rotation;
        // the reference controller holds q_ref and resumes on later ticks.
        // Independently reproduced against original DualArmController trace.
        if(last.status=="v131_cartesian_otg_rejected" && last.joints_rad.allFinite() &&
           (last.joints_rad-current).norm()<1e-10 && ++transient_otg_rejections<=30)
          continue;
        throw std::runtime_error(
                scenario.name + " 返回了不可接受的 QP 解：" + last.status);
      }
      if ((last.joints_rad - current).cwiseAbs().maxCoeff() > 0.02 + 1.0e-9) {
...
int main(int argc, char ** argv)
{
  try {
    ...
    settings.control_period_s = 1.0 / 200.0;
    settings.maximum_joint_step_rad = 0.02;
    ...
    settings.qp_right_nominal_rad = joints_from_degrees(
      {-55.0, -65.0, 70.0, -60.0, -60.0, 0.0, 0.0});
    tianji_teleop::ArmIkBackendOptions options;
    options.urdf_path = argv[1];
    auto backend = tianji_teleop::create_arm_ik_solver("pico_ee_dexhand_qp", options, settings);
    auto& solver = *backend;
    std::vector<double> solve_times_us;
    std::cout << "left arm\n";
    run_side(

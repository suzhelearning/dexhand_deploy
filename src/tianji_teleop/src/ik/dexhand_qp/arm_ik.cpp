...
struct DexhandQpArmIk::Impl {
  ...
  Impl(const std::string& urdf, const IkSettings& s) : settings(s) {
    pin::urdf::buildModel(urdf, model);
    data = std::make_unique<pin::Data>(model);
    const char* override_path=std::getenv("TIANJI_V131_MODEL");
    const auto model_path=override_path?std::filesystem::path(override_path):
      std::filesystem::path(urdf).parent_path().parent_path().parent_path()/
        "v131/marvin_m6_qp_pico_fast_kinematics.xml";
    robot=std::make_unique<ref::MujocoRobot>(model_path.string());
...
DexhandQpArmIk::DexhandQpArmIk(const std::string& u,const IkSettings& s):impl_(std::make_unique<Impl>(u,s)) {}

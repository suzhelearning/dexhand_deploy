...
#include "tianji_mapped_palm/pico_trace_recorder.hpp"
#include "tianji_mapped_palm/telemetry.hpp"
...
struct PicoUdpReceiverOptions {
  ...
  // When enabled, the stream gate validates the same corrected-skeleton EE
  // pose that the mapped PICO controller consumes.  Legacy modes keep the
  // packet-pose gate by default.

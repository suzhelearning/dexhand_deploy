...
from ..protocol.messages import (
    ...
    RawPicoControllerSample, SessionState, ARM_JOINT_NAMES, HAND_JOINT_NAMES,
    ALL_ARM_JOINT_NAMES,
)

SCHEMA_NAME = "tianji-teleop-session"
SCHEMA_VERSION = "1.0"
SOURCE_TYPES = frozenset({"pico_controller", "mocap_live", "h5_replay", "target_replay", "joint_replay"})
SIDES = ("left", "right")
...
class SessionH5Writer:
    ...
    def append_raw_pico(self, sample: RawPicoControllerSample, received_time_ns: int | None = None) -> None:
        if not isinstance(sample, RawPicoControllerSample): raise TypeError("sample must be RawPicoControllerSample")
        group = self._file["raw/pico_controller"]; self._common(group, self._record_time(received_time_ns), sample.source_timestamp_ns, sample.envelope.publisher_instance_id, sample.envelope.sequence); _append(group["left_pose"], sample.left_pose); _append(group["right_pose"], sample.right_pose); _append(group["right_a_pressed"], sample.right_a_pressed); self._maybe_flush()
    ...
    append_raw_pico_controller = append_raw_pico
...
class SessionH5Reader:
    ...
    def _validate(self, allow_incomplete: bool) -> None:
        ...
        if attrs.get("source_type") not in SOURCE_TYPES: raise SessionH5Error("invalid source_type")
        ...
        if set(self.file["raw"].keys()) != {"pico_controller", "mocap_live", "h5_replay"} or set(self.file["target"].keys()) != {"arm", "hand"} or set(self.file["joint"].keys()) != {"command", "state"} or set(self.file["meta"].keys()) != {"session_events"}: raise SessionH5Error("invalid fixed group set")
        ...
        self._validate_group("raw/pico_controller", scalar_i + scalar_b + instance + (("sequence", (), np.int64), ("left_pose", (7,), np.float64), ("right_pose", (7,), np.float64), ("right_a_pressed", (), np.bool_)))
    ...
    def read_raw_pico(self) -> list[dict[str, Any]]: rows = self._source(self._row_group(self.file["raw/pico_controller"])); [row.update(source_type="pico_controller") for row in rows]; return rows
    ...
    def read_session_events(self) -> list[dict[str, Any]]:
        ...
        return rows
    read_raw_pico_controller = read_raw_pico

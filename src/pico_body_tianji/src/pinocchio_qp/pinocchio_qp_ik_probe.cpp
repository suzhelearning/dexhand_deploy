...
pico_body_tianji::ArmJointVector joints_from_degrees(
...
void run_side(
  pico_body_tianji::PinocchioQpArmIk & solver,
  pico_body_tianji::ArmSide side,
  const pico_body_tianji::ArmJointVector & home,
  std::vector<double> & solve_times_us)
{
  ...
  for (const Scenario & scenario : scenarios) {
    pico_body_tianji::ArmJointVector current = home;
    ...
    pico_body_tianji::IkResult last;
...
int main(int argc, char ** argv)
{
  try {
    ...
    settings.qp_right_nominal_rad = joints_from_degrees(
      {-55.0, -65.0, 70.0, -60.0, -60.0, 0.0, 0.0});
    pico_body_tianji::PinocchioQpArmIk solver(argv[1], settings);
    std::vector<double> solve_times_us;
    std::cout << "left arm\n";
    run_side(
      solver,
